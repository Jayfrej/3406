...
class CopyHistory:
    ...
    def _normalize_event(self, event: Dict) -> Dict:
        ...
        # สร้าง action string โดยรวม order_type (เฉพาะ pending orders)
        action = str(event.get('action', 'UNKNOWN')).upper()
        order_type = str(event.get('order_type', '')).lower()

        # ถ้าเป็น pending order (stop/limit) ให้รวม order_type เข้าไป
        if order_type in ['stop', 'limit']:
            action = f"{action} {order_type.upper()}"

        # Trading info
        normalized['master'] = str(event.get('master', '-'))
        normalized['slave'] = str(event.get('slave', '-'))
        normalized['action'] = action  # ใช้ action ที่รวม order_type แล้ว
        ...
        if 'tp' in event:
            try:
                normalized['tp'] = float(event['tp'])
            except (ValueError, TypeError):
                normalized['tp'] = event.get('tp', '')
        else:
            normalized['tp'] = ''

        if 'sl' in event:
            try:
                normalized['sl'] = float(event['sl'])
            except (ValueError, TypeError):
                normalized['sl'] = event.get('sl', '')
        else:
            normalized['sl'] = ''
    ...
    def delete_by_account(self, account: str) -> int:
        """
        ลบประวัติที่เกี่ยวข้องกับ account ที่ระบุ (ทั้ง master และ slave)

        Args:
            account: หมายเลข account ที่ต้องการลบ history
        ...
        try:
            ...
            with self._lock:
                ...
                # กรอง events ที่ไม่ใช่ account นี้
                filtered = [
                    e for e in self.buffer
                    if str(e.get('master')) != account and str(e.get('slave')) != account
                ]
                ...
                logger.info(f"[COPY_HISTORY] Deleted {deleted_count} events for account {account}")
                return deleted_count

        except Exception as e:
            logger.error(f"[COPY_HISTORY] Failed to delete by account: {e}", exc_info=True)
            return 0
