...
Please check your system immediately if this is an error notification.
    ...
    def send_copy_pair_created_alert(self, master_account: str, slave_account: str,
                                     master_nickname: str = "", slave_nickname: str = "",
                                     settings: Dict = None):
        """Send alert when a new copy trading pair is created"""
        master_display = f"{master_nickname} ({master_account})" if master_nickname else master_account
        slave_display = f"{slave_nickname} ({slave_account})" if slave_nickname else slave_account
        ...
        if settings:
            ...
            settings_info = f"""
Settings:
- Volume Mode: {volume_mode.upper()}
- Multiplier: {multiplier}
- Auto Map Symbol: {'Yes' if settings.get('auto_map_symbol', True) else 'No'}
- Copy TP/SL: {'Yes' if settings.get('copy_psl', True) else 'No'}
            """

        message = f"""
✅ New Copy Trading Pair Created Successfully!

Master Account: {master_display}
Slave Account: {slave_display}
Created: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
Status: Active
{settings_info}
The copy trading pair is now active and ready to process signals.
        """
        self.send_alert("✅ Copy Pair Created", message.strip(), "normal")

    def send_copy_pair_updated_alert(self, pair_id: str, master_account: str,
                                     slave_account: str, updates: Dict):
        """Send alert when a copy trading pair is updated"""
        update_details = []
        if 'settings' in updates:
            update_details.append("Settings modified")
        if 'master_account' in updates or 'slave_account' in updates:
            update_details.append("Account information changed")
        if 'status' in updates:
            update_details.append(f"Status changed to: {updates['status']}")

        message = f"""
🔄 Copy Trading Pair Updated

Pair ID: {pair_id}
Master Account: {master_account}
Slave Account: {slave_account}
Updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

Changes:
{chr(10).join(f'- {detail}' for detail in update_details)}

The pair configuration has been updated successfully.
        """
        self.send_alert("🔄 Copy Pair Updated", message.strip(), "low")

    def send_copy_pair_deleted_alert(self, master_account: str, slave_account: str):
        """Send alert when a copy trading pair is deleted"""
        message = f"""
🗑️ Copy Trading Pair Deleted

Master Account: {master_account}
Slave Account: {slave_account}
Deleted: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

The copy trading relationship has been terminated.
No further signals will be copied between these accounts.
        """
        self.send_alert("🗑️ Copy Pair Deleted", message.strip(), "normal")

    def send_copy_trading_error_alert(self, error_type: str, master_account: str,
                                      slave_account: str, error_message: str,
                                      signal_data: Dict = None):
        """Send alert when copy trading error occurs"""
        signal_info = ""
        if signal_data:
            signal_info = f"""
Signal Details:
...
- Event: {signal_data.get('event', 'N/A')}
...
Please check the system logs for more details.
        """
        self.send_alert(f"❌ Copy Trading Error - {error_type}", message.strip(), "high")

    def send_copy_signal_processed_alert(self, master_account: str,
                                        success_count: int, failed_count: int,
                                        skipped_count: int, total_slaves: int,
                                        signal_data: Dict = None):
        """Send alert for copy trading signal processing summary (optional - for important signals)"""
        if success_count == total_slaves:
            emoji = "✅"
            priority = "low"
        ...
        if signal_data:
            signal_info = f"""
Signal: {signal_data.get('event', 'N/A')} {signal_data.get('symbol', 'N/A')} {signal_data.get('volume', 'N/A')} lots
            """

        message = f"""
{emoji} Copy Trading Signal Processed
...
{signal_info}
Results:
- Successful: {success_count}/{total_slaves}
- Failed: {failed_count}/{total_slaves}
- Skipped: {skipped_count}/{total_slaves}

Check logs for detailed information.
        """
        self.send_alert(f"{emoji} Copy Signal - {status}", message.strip(), priority)
