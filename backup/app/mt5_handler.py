...
class MT5Handler:
    ...
    def __init__(self):
        ...
        if self.mt5_available:
            logger.info("[MT5_HANDLER] MetaTrader5 library available for direct trading")
        else:
            logger.info("[MT5_HANDLER] MetaTrader5 library not available - will use Command Queue API")
    
    def send_trading_signal(self, account: str, command: Dict, instance_path: str) -> Dict:
        """
        Send trading signal to MT5 instance

        Note: This method is for direct MT5 API trading only.
        For webhook/copy trading, use command_queue.add_command() instead.
        """
        if self.mt5_available:
            return self._send_direct_to_mt5(account, command, instance_path)
        else:
            # MT5 library not available - commands should go through API queue instead
            logger.warning("[MT5_HANDLER] MT5 library not available. Use command_queue.add_command() for API-based trading.")
            return {'success': False, 'error': 'MT5 library not available. Use API queue instead.'}
